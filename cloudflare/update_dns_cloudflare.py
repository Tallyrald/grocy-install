#!/usr/bin/env python3
# Script by Valentin Heidelberger https://github.com/va1entin/tools/blob/master/cloudflare-update-record/cloudflare-update-record.py
...
def get_config(args):
    ...
    request_successful, record_id_response = make_request('get', f'https://api.cloudflare.com/client/v4/zones/{config["zone_identifier"]}/dns_records?name={config["record_name"]}&type=A', headers={"Authorization": f"Bearer {config['read_token']}", "Content-Type": "application/json"}, exit_on_fail=True)
    try:
